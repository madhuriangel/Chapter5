...
- minDuration=5
- joinAcrossGaps=True
- maxGap=4 with 6-day mean checks around gaps (as in your code)
"""
...
# climatology baseline (as you requested)
...
# must be >= 6 to support "6-day mean" checks; 30 is safe but still light.
...
def compute_climatology_threshold(sst_da: xr.DataArray,
    ...
    Feb 29 is interpolated like in your detect(
    """
    ...
    # Feb 29 interpolation like your code :contentReference[oaicite:5]{index=5}
    thr[FEB29_DOY - 1]  = 0.5 * thr[FEB29_DOY - 2]  + 0.5 * thr[FEB29_DOY]
    seas[FEB29_DOY - 1] = 0.5 * seas[FEB29_DOY - 2] + 0.5 * seas[FEB29_DOY]

    # Smooth (periodic) like your detect() runavg()
...
def detect_mhw_flag_1d(temp, thr,
    ...
    # gap-merge with 6-day mean checks (as in your code)
...
# Print or save the first few values for check
    ...
    for ii, init_dt in enumerate(tqdm(init_times, desc="init_time")):
        ...
        # (assumes daily continuous time axis)
        try:
