...
Option A: Precompute "pseudo-realtime" 2024 predictions using a trained 1-step CNN2D-LSTM
...
def to_norm_full(frames_phys, valid_idx_flat, scaler, mask_hw, H, W):
    ...
    returns full normalized frames (T,H,W) with zeros elsewhere
    """
    ...
    # Fill any remaining NaNs with scaler mean -> normalized 0
    if not np.isfinite(valid).all():
...
# =========================
# Save to Zarr (recommended for fast web reads)
# =========================
