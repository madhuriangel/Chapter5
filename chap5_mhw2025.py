...
Compute daily anomaly / intensity / mhw_flag for CNN2D-LSTM forecasts (2025 demo).

Logic
- Uses observed SST 1982-2025 file
- Uses 2025 forecast Zarr
- Uses climatology baseline 1991-2024
"""
...
# ONLY PLACE TO EDIT
# =========================
OBS_SST_NC  = "noaa_icesmi_combinefile_FINAL_1res1982_2025.nc"

PRED_ZARR   = "CNN2DLSTM_PRECOMP_2025_SEQ15/cnn2dlstm_pred_2025_lead0to6.zarr"
PRED_VAR    = "sst_pred"

OUT_DIR     = "CNN2DLSTM_MHW_2025_SEQ15"
OUT_ZARR    = os.path.join(OUT_DIR, "cnn2dlstm_mhw_2025_lead0to6.zarr")
...
# must be >= 6 to support "6-day mean" checks;
BUFFER_BEFORE_DAYS = 30


# =========================
# Utils: leap-year DOY mapping
# =========================
...
def compute_climatology_threshold(sst_da: xr.DataArray,
    ...
    # Build each doy using a window of DOYs
    for d in range(1, 367):
        if d == FEB29_DOY:
            continue
...
# Print or save the first few values for check
    ...
    for ii, init_dt in enumerate(tqdm(init_times, desc="init_time")):
        ...
        for k in range(Nv):
            ...
            # keep forecast portion (last 7 days)
